		...
		rules : {
			uploadFile :{
				...
				remote : { // 验证用户名是否存在
					...
					data : {
						id:function(){
							return $("#update_works_id").val();//这个地方在更新和新增时验证的元素ID是不一样的
						},
						name : function() {
...
function suredeleteWorks()
{
	...
	$.post("/worksApproval/portalDeleteWorks",{ ids:ids },
			function(data){
		$("#alertMessage").html(data[0]);
		setTimeout(function(){
			$('#alertModal').modal('toggle');
		}, 4600);
		window.location.href="/worksApproval/portalUserCenter.html?t="+(new Date().getTime());
	});	
...
function updateWorks()
{
	...
	if (tr_input$.length == 0) {
		...
		works.uploadFile = $(tr$[3]).children("a").html().trim();		
...
/*
 * function testgetData() { alert(11111); //var tr_input$ = $("div#updateWorks + form +
