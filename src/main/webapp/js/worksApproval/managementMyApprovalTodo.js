...
{
	//alert(12121212);
	$(".approval_setting").click(function(){
		...
		var t = $(this).parent().siblings()[3];
		var worksName = $(t).children("a").html().trim();
		//<a href="http://localhost:7070/worksApproval/downloadWorks2?downloadId=7&amp;id=29&amp;backurl=/WEB-INF/page/managementMyApprovalTodo.jsp">spring-hibernate.xml</a>
		//worksName
		/*String ss = worksName;
		ss.substring(ss.indexOf(">")+1, ss.lastIndexOf("<"));
		worksName = ss;*/
		var wid = $(this).parent().siblings()[0];
		var worksId = $(wid).html().trim();
		
		//alert(worksId);
		//alert(worksName);
		$("#work_id").val(worksId);
		$("#work_name").val(worksName);
		/*alert(worksId);
		alert(worksName);
		*/
		///pageDetail
 ...
 /*$.post(url,{
		...
		*/
		$('#approval_setting_modal').modal('toggle');
		//showPageDetail_select 
	});
}
